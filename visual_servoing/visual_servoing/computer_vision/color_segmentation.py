...
def detect_orange(img):
        ...
        kernel = np.ones((5, 5), np.uint8)
        mask_erode = cv2.erode(mask_inrange, kernel, iterations=1)
        mask_dilate = cv2.dilate(mask_erode, kernel, iterations=2)
        cv2.imshow("3 - Mask (erode + dilate)", mask_dilate)
        ...
        cv2.imshow("4 - Final with Bounding Box", img)
        cv2.waitKey(1)  # Required for OpenCV to update windows
...
if __name__ == "__main__":
    ...
    img = cv2.imread("test_images_cone/test12.jpg") # change to problem files
    if img is None:
